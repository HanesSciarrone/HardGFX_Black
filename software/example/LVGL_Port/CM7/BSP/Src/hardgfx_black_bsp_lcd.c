...
void HardGFX_BSP_LCD_DisplayOff(void)
{
    ...
    /* Power-off LCD_DISP pin */
    HAL_GPIO_WritePin(LCD_DISP_GPIO_PORT, LCD_DISP_PIN, GPIO_PIN_RESET);
    /* Power-off LCD_BL pin */
...
/**
  * @brief  Get address of LCD layer.
  *
  * @param[in]  xPos    X position of the area to be drawn.
  * @param[in]  yPos    Y position of the area to be drawn.
  *
  * @retval Returns the address of the LCD buffer to be drawn.
  */
uint32_t HardGFX_BSP_LCD_GetAddressLayer(const uint32_t xPos, const uint32_t yPos)
{
    return lcdHandler.LayerCfg[lcdObject.activeLayer].FBStartAdress + (((lcdObject.xSize * yPos) + xPos) * lcdObject.bppFactor);
}
...
static bool convertLineToRGB(uint32_t *bufferSource, uint32_t *bufferDestination, const uint32_t xSize, const uint32_t colorMode)
{
    ...
    switch(lcdObject.pixelFormat)
    {
        ...
        case HARDGFX_LCD_PIXEL_FORMAT_ARGB8888:
        default:
        {
